<template>
    <div class="brand-wrapper f-skin-bg pd-20">
        ...
        >
            <el-table-column align="center" label="品牌名称" prop="cName" />
            <el-table-column align="center" label="品牌英文名称" prop="eName" />
            <el-table-column align="center" label="品牌logo图" prop="images">
                <template slot-scope="scope">
                    <img :src="`${scope.row.image + '?'+ Date.now()}`" class="t-img--w200">
                </template>
            </el-table-column>
...
import { AddBrandModal } from './popup'
...
export default {
    ...
    methods: {
        // 添加品牌
        handleCreateBrand() {
            AddBrandModal.popup().then(res => {})
        },
        // 编辑品牌
        handleEditBrand(ele) {
            AddBrandModal.popup({ formObj: { ...ele } }).then(res => {}, err => {
                console.info(111111)
            }).catch(err => {
                console.info(22222)
            })
